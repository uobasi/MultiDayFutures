...
def update_graph_live(n_intervals, sname, stored_data, interval_time, previous_stkName): #,previous_stkName
    ...
    fig.add_trace(go.Bar(x=pd.Series([i for i in range(len(df))]), y=df['buyCount'], marker_color='teal', name='buyCount'), row=3, col=1) #tst
    fig.add_trace(go.Bar(x=pd.Series([i for i in range(len(df))]), y=df['SellCount'], marker_color='crimson', name='SellCount'), row=3, col=1) #tst
    ...
    fig.update_yaxes(
        range=[
            min([i for i in combined_df['buyCount'][int(len(df) * 0.90):len(df)]] + [i for i in combined_df['sellCount'][int(len(df) * 0.90):len(df)]]), 
            max([i for i in combined_df['sellCount'][int(len(df) * 0.90):len(df)]] + [i for i in combined_df['buyCount'][int(len(df) * 0.90):len(df)]])
        ],
        row=3, col=1
    )
