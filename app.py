...
def update_graph_live(n_intervals, sname, stored_data, interval_time, previous_stkName): #,previous_stkName
    ...
    fig = make_subplots(rows=4, cols=1, shared_xaxes=True, shared_yaxes=True,
                        specs=[[{}],
                         ...
                         row_width=[0.15,0.15,0.10,0.60,] ) #,row_width=[0.30, 0.70,] column_widths=[0.85,0.15], 
    ...
    fig.add_trace(go.Scatter(x=pd.Series([i for i in range(len(df))]), y=df['buyDecimal'], marker_color='teal'), row=2, col=1) #tst
    fig.add_trace(go.Scatter(x=pd.Series([i for i in range(len(df))]), y=df['sellDecimal'], marker_color='crimson'), row=2, col=1)
    ...
    fig.add_trace(go.Scatter(x=pd.Series([i for i in range(len(df))]), y=df['prevDayLVA'], mode='lines', name='prevDayLVA', line=dict(color='purple'))) 
