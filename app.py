...
def update_graph_live(n_intervals, sname, stored_data, interval_time, previous_stkName): #,previous_stkName
    ...
    fig.add_trace(go.Scatter(x=pd.Series([i for i in range(len(df))]), y=df['buyCount5C'], marker_color='teal', name='buyCount5C'), row=2, col=1) #tst
    fig.add_trace(go.Scatter(x=pd.Series([i for i in range(len(df))]), y=df['sellCount5C'], marker_color='crimson', name='sellCount5C'), row=2, col=1)
